...
def update_student_balance(sender, instance, created, **kwargs):
    """Update student balance after payment is recorded
    
    Payment priority (applies globally across all terms):
    1. Oldest unpaid ARREARS first (debt from previous terms)
    2. Then current term fee
    3. Then newer terms
    """
    if not created:
        return  # only update on newly created payments
    ...
    remaining_payment = Decimal(str(instance.amount))
    
    # Get ALL balances for this student, ordered by year and term (oldest first)
    all_balances = StudentBalance.objects.filter(
        student=instance.student
    ).order_by('term__academic_year', 'term__term')
    
    # PRIORITY 1: Pay off ARREARS from oldest terms first
    # Arrears are unpaid balances from previous terms
    for balance in all_balances:
        if remaining_payment <= 0:
            break
        
        # Calculate how much of this term's balance is arrears (what was carried forward)
        # Arrears = any unpaid balance from before this term
        if balance.previous_arrears > 0:
            # This balance still has unpaid arrears
            amount_already_paid_to_arrears = min(
                Decimal(str(balance.amount_paid)), 
                Decimal(str(balance.previous_arrears))
            )
            arrears_still_owed = Decimal(str(balance.previous_arrears)) - amount_already_paid_to_arrears
            
            if arrears_still_owed > 0:
                # Apply payment to cover these arrears
                amount_to_apply = min(remaining_payment, arrears_still_owed)
                balance.amount_paid = Decimal(str(balance.amount_paid)) + amount_to_apply
                balance.save()
                remaining_payment -= amount_to_apply
    
    # PRIORITY 2: Pay off current term fees (if any payment remains)
    for balance in all_balances:
        if remaining_payment <= 0:
            break
        
        # Calculate how much of the term fee is still unpaid
        # All arrears have been paid (from priority 1), so remaining payment goes to term fee
        term_fee_paid = max(
            Decimal('0'),
            Decimal(str(balance.amount_paid)) - Decimal(str(balance.previous_arrears))
        )
        term_fee_still_owed = Decimal(str(balance.term_fee)) - term_fee_paid
        
        if term_fee_still_owed > 0:
            # Apply payment to cover term fee
            amount_to_apply = min(remaining_payment, term_fee_still_owed)
            balance.amount_paid = Decimal(str(balance.amount_paid)) + amount_to_apply
            balance.save()
            remaining_payment -= amount_to_apply
    
    # Check if student should be archived (graduated with all arrears paid)
    instance.student.check_and_archive()
